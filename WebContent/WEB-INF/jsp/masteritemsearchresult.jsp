...
<body>
	...
	<div class="container">
		<div class="row center">
			<h5 class=" col s12 light">登録商品一覧</h5>
			<p>
				検索結果${itemCount}件
			</p>
		</div>
		<div class="section">
			...
			<div class="row">
				...
				<div class="col s12 m3">
					<div class="card">
						<div class="card-image">
							<a href="MasterItemDetail?item_id=${item.id}"><img src="img/${item.fileName}"></a>
						</div>
				...
				<c:if test="${(status.index + 1) % 4 == 0}"><!-- 四つ表示したら１行空けるの意味 --> //ここなに
			</div>
			<div class="row">
				</c:if>
				</c:forEach>
		...
		<div class="row center">
			<ul class="pagination">
				...
				<c:forEach begin="${(pageNum - 5) > 0 ? pageNum - 5 : 1}" end="${(pageNum + 5) > pageMax ? pageMax : pageNum + 5}" step="1" varStatus="status">
					<c:if test="${pageNum == status.index }"> class="active" </c:if><a href="ItemSearchResult?search_word=${searchWord}&page_num=${status.index}">${status.index}</a></li>
				</c:forEach>
